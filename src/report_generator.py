...
class ReportGenerator:
    ...
    def _get_safe_trial_balance(self):
        """Helper untuk mendapatkan Trial Balance yang DIJAMIN memiliki semua kolom yang dibutuhkan."""
        df = self.db.get_trial_balance()
        # Daftar kolom yang wajib ada
        required_cols = ['balance', 'debit', 'credit', 'total_debit', 'total_credit', 'name', 'type', 'category', 'code', 'id']
        
        # Pastikan setiap kolom ada, jika tidak ada buat kolom kosong/nol
        for col in required_cols:
            if col not in df.columns:
                if col in ['balance', 'debit', 'credit', 'total_debit', 'total_credit']:
                    df[col] = 0.0
                else:
                    df[col] = "-"
        
        # Pastikan tidak ada nilai NaN pada kolom angka agar tidak crash saat dijumlahkan
        num_cols = ['balance', 'debit', 'credit', 'total_debit', 'total_credit']
        df[num_cols] = df[num_cols].fillna(0.0)
        
        return df

    def get_trial_balance_report(self):
        df = self._get_safe_trial_balance()
    ...
    def get_isak35_financial_position(self):
        ...
        # Beban dipisahkan berdasarkan kategori COA-nya
        ...
        # Jika ada beban tanpa kategori, masukkan ke Tanpa Pembatasan
        ...
        return {
            'assets': assets_df[['name', 'balance']].to_dict('records') + contra_assets_df[['name', 'balance']].to_dict('records'),
    ...
    def get_cash_flow_report(self):
        df_tb = self._get_safe_trial_balance()
        ...
        total_cash_end = cash_accounts['balance'].sum() if not cash_accounts.empty else 0.0
    ...
    def export_annual_report_to_pdf(self, file_path, year):
        try:
            ...
            t_vm = Table(vm_rows, colWidths=[1*inch, 5.2*inch])
            t_vm.setStyle(TableStyle([('GRID',(0,0),(-1,-1),0.5,colors.grey), ('BACKGROUND',(0,0),(0,-1),colors.lightgrey), ('FONTNAME',(0,0),(0,-1),'Helvetica-Bold'), ('VALIGN',(0,0),(-1,-1),'TOP')]))
            elements.append(t_vm)
            ...
            t_org = Table(data_org, colWidths=[2.5*inch, 3.7*inch])
            t_org.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),colors.navy), ('TEXTCOLOR',(0,0),(-1,0),colors.whitesmoke), ('GRID',(0,0),(-1,-1),0.5,colors.grey)]))
            ...
            t_kin = Table(data_kinerja, colWidths=[1.5*inch, 4.7*inch])
            t_kin.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),colors.navy), ('TEXTCOLOR',(0,0),(-1,0),colors.whitesmoke), ('GRID',(0,0),(-1,-1),0.5,colors.grey), ('VALIGN',(0,0),(-1,-1),'TOP')]))
            elements.append(t_kin)

            # HALAMAN 2-4: KEUANGAN
            elements.append(PageBreak())
            elements.append(Paragraph("<b>IV. LAPORAN KEUANGAN</b>", styles['Heading2']))
            
            # A. POSISI KEUANGAN
            elements.append(Paragraph("A. Laporan Posisi Keuangan", styles['Heading3']))
            pos = self.get_isak35_financial_position()
            rows = [["Keterangan", "Jumlah (Rp)"], ["ASET", ""]]
            ...
            t_p = Table(rows, colWidths=[4.7*inch, 1.5*inch])
            t_p.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),colors.navy), ('TEXTCOLOR',(0,0),(-1,0),colors.whitesmoke), ('GRID',(0,0),(-1,-1),0.5,colors.grey), ('ALIGN',(1,1),(-1,-1),'RIGHT')]))
            ...
            # B. AKTIVITAS
            elements.append(PageBreak())
            elements.append(Paragraph("B. Laporan Aktivitas", styles['Heading3']))
            akt = self.get_statement_of_activities()
            rows = [["Keterangan", "Jumlah (Rp)"], ["PENDAPATAN", ""]]
            ...
            t_a = Table(rows, colWidths=[4.7*inch, 1.5*inch])
            t_a.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),colors.navy), ('TEXTCOLOR',(0,0),(-1,0),colors.whitesmoke), ('GRID',(0,0),(-1,-1),0.5,colors.grey), ('ALIGN',(1,1),(-1,-1),'RIGHT')]))
            for i, r in enumerate(rows):
            ...
            # C. PERUBAHAN ASET NETO
            elements.append(PageBreak())
            elements.append(Paragraph("C. Laporan Perubahan Aset Neto", styles['Heading3']))
            per = self.get_changes_in_net_assets_report()
            p_rows = [["Uraian", "Tanpa Pembatasan", "Dengan", "Total"]]
            ...
            tp = Table(p_rows, colWidths=[3.2*inch, 1.0*inch, 1.0*inch, 1.0*inch])
            tp.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),colors.navy), ('TEXTCOLOR',(0,0),(-1,0),colors.whitesmoke), ('GRID',(0,0),(-1,-1),0.5,colors.grey), ('ALIGN',(1,1),(-1,-1),'RIGHT'), ('FONTSIZE',(0,0),(-1,-1),8)]))
            elements.append(tp)
            
            # D. ARUS KAS
            elements.append(Spacer(1, 20))
            elements.append(Paragraph("D. Laporan Arus Kas", styles['Heading3']))
            cf = self.get_cash_flow_report()
            c_rows = [["Keterangan", "Jumlah (Rp)"]]
            ...
            tc = Table(c_rows, colWidths=[4.7*inch, 1.5*inch])
            tc.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),colors.navy), ('TEXTCOLOR',(0,0),(-1,0),colors.whitesmoke), ('GRID',(0,0),(-1,-1),0.5,colors.grey), ('ALIGN',(1,1),(-1,-1),'RIGHT')]))
            elements.append(tc)

            # HALAMAN EVALUASI
            elements.append(PageBreak())
            elements.append(Paragraph("<b>V. EVALUASI, KENDALA & RENCANA STRATEGIS</b>", styles['Heading2']))
            ...
            t_ev = Table(ev_table_data, colWidths=[0.4*inch, 1.9*inch, 2.0*inch, 1.9*inch])
            t_ev.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),colors.navy), ('TEXTCOLOR',(0,0),(-1,0),colors.whitesmoke), ('GRID',(0,0),(-1,-1),0.5,colors.grey), ('VALIGN',(0,0),(-1,-1),'TOP'), ('FONTSIZE',(0,0),(-1,-1),8)]))
            ...
            sig_data1 = [["Menyetujui,", "Meninjau,"], ["Ketua Pembina", "Ketua Pengawas"], ["", ""], [f"( {extracted['pembina']} )", f"( {extracted['pengawas']} )"]]
            t_sig1 = Table(sig_data1, colWidths=[3.2*inch, 3.2*inch], rowHeights=[20, 20, 60, 20])
            t_sig1.setStyle(TableStyle([('ALIGN', (0,0), (-1,-1), 'CENTER'), ('VALIGN', (0,0), (-1,-1), 'TOP'), ('FONTNAME', (0,1), (-1,1), 'Helvetica-Bold')]))
            elements.append(t_sig1); elements.append(Spacer(1, 35))
            sig_data2 = [["Mengetahui,", "Dibuat Oleh,"], ["Ketua Pengurus", "Bendahara"], ["", ""], [f"( {extracted['ketua']} )", f"( {extracted['bendahara']} )"]]
            t_sig2 = Table(sig_data2, colWidths=[3.2*inch, 3.2*inch], rowHeights=[20, 20, 60, 20])
            t_sig2.setStyle(TableStyle([('ALIGN', (0,0), (-1,-1), 'CENTER'), ('VALIGN', (0,0), (-1,-1), 'TOP'), ('FONTNAME', (0,1), (-1,1), 'Helvetica-Bold')]))
